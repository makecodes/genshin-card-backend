...
class RiotGamesClient:
    ...
    def get(self, endpoint: str, base_url: str = None, params: dict = None):
        return self.session.get(
            f"{base_url or self.api_url}/{endpoint}", params=params
        ).json()
...
class LeagueOfLegendsClient(RiotGamesClient):
    ...
    def champions(
        self, region: str = "br", language: str = "pt_BR", version: str = None
    ) -> dict:
        if not version:
        ...
        return self.session.get(
            f"http://ddragon.leagueoflegends.com/cdn/{version}/data/{language}/champion.json"
        ).json()
